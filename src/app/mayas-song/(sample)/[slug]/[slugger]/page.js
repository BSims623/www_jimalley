...
export async function generateMetadata({ params }) {
    ...
    return {
        title: chapter,
        description: `Read ${chapter.toLowerCase()}, ${pageNumber} of "Maya's Song".`,
    };
}

export default async function Page({ params }) {
    const { slug, slugger } = params;
    const chapters = await getChapters('/stories/mayas_song.md');
    ...
    return (
        <div className={styles.main_container}>
            ...
            {numberOfPages > 1 && <Pagination numberOfPages={numberOfPages} currentPage={pageNumber} story={slug} thePath={'/mayas-song'} />}
        </div>
    )
}
